...
if /i "%PLAYER_TYPE%"=="sa" (
  if exist "%REPO_ROOT%\tools\runtime\flashplayer_sa.exe" set "PLAYER=%REPO_ROOT%\tools\runtime\flashplayer_sa.exe"
)
...
for /l %%W in (1,1,30) do (
  ...
  tasklist /fi "WINDOWTITLE eq !SERVER_TITLE!" /fo csv /nh 2>nul | findstr /i /c:"cmd.exe" >nul
  if errorlevel 1 (
    del /q "%TEMP%\sa-collab-status-%~1.tmp" >nul 2>nul
    exit /b 2
  )
...
echo Missing development build. Run: powershell -File scripts\dev.ps1 build
exit /b 1
