...
> ✅ LV4/LV5 碎屑三帧已导入（2026-09-15，画师交付图特效1/2/3 对位 f3/f4/f5，原样调色零改写；火焰回归 depth4 排布）——sub1130=**17880730**，待实机确认，详见 `LV4LV5碎屑三帧-画师素材分析-20260915\`
...
- **开火音效起始帧 f3→f5（2026-09-09 补充决策，同文档五之三节）**：用户实测 LV5"音效先于火光"。量化：五级音效均在 f3、火焰显示 f5~f11（峰值 f7~f8）、间隔恒定；LV4/LV5 因无闪屑层 f2~f4 为死帧，脱节感最强。另测得开火音效时长 1.924s（≈58 帧）横跨整个攻击周期，为周期性长音层。修改=五级 StartSoundTag 移至 f5 帧内容（纯位置移动），回读断言音效帧=f5+剔除 StartSound 后逐元素一致；sub1130=`8D885B69` 三处同步+manifest 更新+构建退出码 0+双自检通过。脚本 `lightningball_sndf5.py`。
...
- **lightningball_lost_restore.py**：两项丢失改动重放恢复脚本（2026-09-15，含底稿哈希断言与五级全等回读断言）。
- **LV4LV5碎屑三帧-画师素材分析-20260915\**：LV4/LV5 碎屑三帧全套档案——导入前分析（结构同构、交付图对位验证）、导入执行总结（三帧对位 f3/f4/f5、预乘 ARGB 实证、像素验证口径）、导入脚本、导入后 12 帧实渲染图；交付图在工作区 `相关素材\守望者\`。
