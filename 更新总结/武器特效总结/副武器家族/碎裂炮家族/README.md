...
> 当前状态：🔍 **2.5 与当今版对比完成（2026-09-10，只读零改动；3.4 按用户指令不在范围）**——先读 `碎裂炮家族2.5与当今版对比总结-20260910.md`：**子弹两版逐像素一致；本体/枪口焰同源；2.5 独有蓄力发光层（GH 全删）＋2.5 开火音（GH 缺失）；受击整套更换且可零成本改绑 `bullet/blueness_energy`**；差异字段=attackGap 0.9→1.4、attackType mixed→boom、reduceRa 0.6→0.7＋经济字段。五项回迁候选待用户决策。
...
| 文件/目录 | 内容 |
|---|---|
| 碎裂炮家族2.5与当今版对比总结-20260910.md | **先读**：2.5 vs 当前版只读对比（元素键 A/B/C 全项；子弹 0% 一致、受击整套更换＋blueness_energy 零成本发现、蓄力层差异、开火音缺失、回迁候选五项） |
| 碎裂炮本体待机两版对比图-20260910.png 等 5 张 | 待机/蓄力段/LV1 全帧/受击/子弹对比证据 |
| 工作区（不入库） | `tmp-soya-family-test\chipped-25-gh-audit\`（两版渲染与像素比对） |
