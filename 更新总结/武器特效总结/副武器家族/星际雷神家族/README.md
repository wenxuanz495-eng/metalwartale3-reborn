...
> 当前状态：✅* **皮肤与发射特效整体替换已构建入库待实测（2026-09-17，V2 修订后）**——先读 `星际雷神家族皮肤与发射特效替换执行总结-20260917.md`（含第五节 V2 修订）。V3 现状（深度重排：导弹层垫底 d1/d2＜本体 d3＜特效 d6＜尾焰/双弧）：lv1/lv2 待机均＝空膛枪体＋待机导弹层（只露弹头，f4 撤/f6 回＝装填），lv2 导弹位模板匹配 (93,16)/(92,45)、lv1 (83,11.5)/(83,39)；`星际雷神.png` 系原武器参考图已弃用；V4＝两级美术以射点为枢轴统一缩放 0.75（锚点不缩不移，炮口仍贴射点，系数可 20%~30% 微调）；累计新 ID 2900~2935；零形态/零数值/零配置、无新导出名；sub1130=D0F0E703、game.swf=F8BCD766 未变；构建 0＋双自检 0。**本家族为当前版独有，旧版均无此武器**，无旧版回迁面。
