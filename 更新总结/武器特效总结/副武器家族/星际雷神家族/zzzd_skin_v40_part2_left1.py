# -*- coding: utf-8 -*-
# 星际雷神 V40 修订（2026-09-18，仅 lv1；部件2 再左移 1px——用户 directive）：
...
assert actual == manifest_line.upper() and actual.startswith("10631D54"), actual[:8]
...
for it in sp121.find('subTags'):
    ...
    if cid == '2931' and d == '1':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '344'
        m.set('translateX', '1580')
        m.set('translateY', '344')
        expect['2931'] = ('1', '1580', '344')
    if cid == '2933' and d == '2':
        m = it.find('matrix')
        assert m.get('translateX') == '1640' and m.get('translateY') == '704'
        m.set('translateX', '1580')
        m.set('translateY', '704')
        expect['2933'] = ('2', '1580', '704')
assert len(expect) == 2, "部件2 放置未找齐"
print("矩阵修改 PASS：2931→(1580,344)、2933→(1580,704)（再左移 1px）")
