...
> 当前状态：✅* **LV1~LV3 射速/蓄力/枪口火焰 2.5 回迁已构建入库待实测（2026-09-12）**——先读 `恶魔牙LV1-3射速蓄力枪口火焰2.5回迁执行总结-20260912.md`：①配置等长替换 lv1~3 attackGap 0.9/attackDelay 0.2（lv4~6 不动）②三级本体时间轴整段回 2.5 模板（递归闭包导入 68 项新 ID 2644~2711；白圈脉冲＋电弧×4＋枪口刀刃能量＋烟团；挂点 110→104；开火音维持 366）③渲染验证=新三级全 11 帧 vs 2.5 逐像素一致、lv4/lv6/黄金/子弹零变化 ④sub1130=8B903B3A、config=58CC22AC、game.swf=5F3D3DDB 三处同步＋manifest＋构建＋`--check sa` 通过。前置：`恶魔牙家族2.5-3.4-当前版对比总结-20260912.md`（三版本只读对比）。
