...
现在 `SoundGroup.playRecommendedPlaylist()` 在主界面 ↔ 战斗上下文变化时给 `playlist/start` 追加 `force=1`，立即切换到目标歌单；同一场景内的播放请求保持不重启语义，显式歌单编辑（`syncCurrentRecommendedPlaylist()`）保持原有恒 `force=1` 语义。
...
- 主界面与战斗上下文切换时立即切到目标歌单，即使当前曲目在目标歌单中也存在；同一场景内的播放请求不重启当前曲目。
