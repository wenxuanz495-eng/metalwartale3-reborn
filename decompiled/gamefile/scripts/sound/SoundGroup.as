package sound
{
   ...
   public class SoundGroup
   {
      ...
      private function syncCurrentRecommendedPlaylist() : *
      {
         ...
         var context:String = Game.gameState == "gaming" || Game.gameState == "gaming2" ? "battle" : "main";
      ...
      public function normalizeNamedPlaylists() : Boolean
      {
         var changed:Boolean = false;
         var playlist:Object = null;
         var original:Array = null;
         var normalized:Array = null;
         if(this.recommendedCatalog.length == 0) return false;
         for each(playlist in this.playlistDefinitions)
         {
            if(!(playlist.tracks is Array)) continue;
            original = playlist.tracks as Array;
            normalized = this.migratePlaylistIDs(original);
            if(normalized.length != original.length)
            {
               playlist.tracks = normalized;
               changed = true;
            }
            else
            {
               for(var i:int = 0; i < normalized.length; i++)
               {
                  if(String(normalized[i]) != String(original[i]))
                  {
                     playlist.tracks = normalized;
                     changed = true;
                     break;
         ...
         if(changed) this.resolveNamedPlaylists();
         return changed;
      }
      ...
      private function canonicalRecommendedTrackID(id:String) : String
      {
         ...
         return this.recommendedCatalog.length == 0 ? id : "";
      }
